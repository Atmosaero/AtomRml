...
void AtomRmlInput::OnDeactivate(Rml::TextInputContext*)
{
    m_activeTxtContext = nullptr;
    AzFramework::InputTextNotificationBus::Handler::BusDisconnect();
}

void AtomRmlInput::OnDestroy(Rml::TextInputContext*)
{
    m_activeTxtContext = nullptr;
    AzFramework::InputTextNotificationBus::Handler::BusDisconnect();
}

bool AtomRmlInput::OnInputChannelEventFiltered(const AzFramework::InputChannel& inputChannel)
{
    const AzFramework::InputChannelId& channelId = inputChannel.GetInputChannelId();
...
void AtomRmlInput::OnInputTextEvent(const AZStd::string& text, bool& consumed)
{
    if (text.empty())
    {
        return;
