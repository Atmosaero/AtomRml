...
namespace AtomRml
{
    ...
    AtomRmlRenderInterface::AtomRmlRenderInterface()
    {
    ...
    void AtomRmlRenderInterface::End()
    {
        ...
        for (auto handle : queuedFreeGeos)
        {
            ...
            if (m_createdThisFrame.contains(handle))
            {
                ...
                if (geo && geo->storageType == AtomRmlStoredGeometry::StorageType::Undecided)
                {
                    geo->storageType = AtomRmlStoredGeometry::StorageType::Transient;
                }
            }
        }
        ...
        for (const auto& cmd : drawCmds)
        {
            ...
            if (geo && geo->storageType == AtomRmlStoredGeometry::StorageType::Undecided)
            {
                geo->storageType = AtomRmlStoredGeometry::StorageType::Persistent;
            }
        }
    ...
    AtomRmlStoredGeometry* AtomRmlRenderInterface::GetStoredGeometry(Rml::CompiledGeometryHandle handle)
    {
        if (!handle)
        {
            return nullptr;
        ...
        return reinterpret_cast<AtomRmlStoredGeometry*>(handle);
    }

    const AtomRmlStoredTexture* AtomRmlRenderInterface::GetStoredTexture(Rml::TextureHandle handle)
    {
        if (!handle)
        {
            return nullptr;
        ...
        return reinterpret_cast<AtomRmlStoredTexture*>(handle);
    }

#pragma region Rml::RenderInterface
    Rml::CompiledGeometryHandle AtomRmlRenderInterface::CompileGeometry(Rml::Span<const Rml::Vertex> vertices,
                                                                      Rml::Span<const int> indices)
    {
        ...
        auto* storedGeo = aznew AtomRmlStoredGeometry();
                                              ...
                                              Rml::TextureHandle texture)
    {
        ...
        {
            drawCmd.drawType = AtomRmlDrawCommand::DrawType::Clipmask;
            drawCmd.clipmask_op = m_clipmaskOperation;
        }
        else
        {
            drawCmd.drawType = AtomRmlDrawCommand::DrawType::Normal;
            drawCmd.clipmask_op = m_clipmaskOperation;
        }
    ...
    Rml::TextureHandle AtomRmlRenderInterface::LoadTexture(Rml::Vector2i& texture_dimensions, const Rml::String& source)
    {
        ...
        AtomRmlStoredTexture* storedTex = aznew AtomRmlStoredTexture();
    ...
    Rml::TextureHandle AtomRmlRenderInterface::GenerateTexture(Rml::Span<const Rml::byte> source,
                                                             Rml::Vector2i source_dimensions)
    {
        ...
        AZStd::string textureName = AZStd::string::format("AtomRml Texture #%p", storedTex);
    ...
    void AtomRmlRenderInterface::ReleaseTexture(Rml::TextureHandle textureId)
    {
        if (!textureId)
        {
            return;
        ...
        auto texture = reinterpret_cast<AtomRmlStoredTexture*>(textureId);
    ...
    void AtomRmlRenderInterface::EnableScissorRegion(bool enable)
    {
        m_scissorEnabled = enable;
    }

    void AtomRmlRenderInterface::SetScissorRegion(Rml::Rectanglei region)
    {
        m_scissorRegion = region;
    }

    void AtomRmlRenderInterface::SetTransform(const Rml::Matrix4f* transform)
    {
        if (transform)
        {
    ...
    void AtomRmlRenderInterface::EnableClipMask(bool enable)
    {
        m_testClipMask = enable;
    }

    void AtomRmlRenderInterface::RenderToClipMask(Rml::ClipMaskOperation operation, Rml::CompiledGeometryHandle geometry,
                                                Rml::Vector2f translation)
    {
        ...
        if (clearClipmask)
        {
            //Submit clear cmd
            AtomRmlDrawCommand drawCmd;
            drawCmd.drawType = AtomRmlDrawCommand::DrawType::ClearClipmask;
    ...
    AZStd::vector<AtomRmlChildPassDrawCommand>& AtomRmlRenderInterface::GetDrawCommands() const
    {
...
#pragma endregion
    ...
    ReusableBuffer* AtomRmlRenderInterface::RequestBuffer(size_t capacity, size_t elementSize)
    {
        ...
        desc.m_elementSize = aznumeric_cast<uint32_t>(elementSize);
    ...
    void AtomRmlRenderInterface::AllocateGPUBuffers()
    {
        ...
        for (const auto& cmd : drawCmds)
        {
            auto* geo = AtomRmlRenderInterface::GetStoredGeometry(cmd.drawCommand.geometryHandle);
            if (geo && geo->storageType == AtomRmlStoredGeometry::StorageType::Transient)
            {
        ...
        for (const auto& cmd : drawCmds)
        {
            ...
            if (geo->storageType == AtomRmlStoredGeometry::StorageType::Persistent)
            {
                ...
                if (geo->vertexBuffer && geo->indexBuffer)
                {
                    ...
                    geo->vertexBufferView = AZ::RHI::StreamBufferView(
                        *geo->vertexBuffer->buffer->GetRHIBuffer(),
                        0,
                        aznumeric_cast<uint32_t>(vertexBytes),
                        sizeof(Rml::Vertex)
                    );
                    ...
                    geo->indexBufferView = AZ::RHI::IndexBufferView(
                        *geo->indexBuffer->buffer->GetRHIBuffer(),
                        0,
                        aznumeric_cast<uint32_t>(indexBytes),
            ...
            else if (geo->storageType == AtomRmlStoredGeometry::StorageType::Transient)
            {
                ...
                geo->vertexBufferView = AZ::RHI::StreamBufferView(
                    *frameInfo.m_sharedVertexBuffer->GetRHIBuffer(),
                    aznumeric_cast<uint32_t>(geo->vertexOffsetInShared * sizeof(Rml::Vertex)),
                    aznumeric_cast<uint32_t>(geo->vertices.size() * sizeof(Rml::Vertex)),
                    sizeof(Rml::Vertex)
                );

                geo->indexBufferView = AZ::RHI::IndexBufferView(
                    *frameInfo.m_sharedIndexBuffer->GetRHIBuffer(),
                    aznumeric_cast<uint32_t>(geo->indexOffsetInShared * sizeof(int)),
                    aznumeric_cast<uint32_t>(geo->indices.size() * sizeof(int)),
                    AZ::RHI::IndexFormat::Uint32
                );
    ...
    void AtomRmlRenderInterface::OnImGuiUpdate()
    {
        ImGui::Begin("AtomRml Render Interface");
        {
            ...
            auto fp = scene->GetFeatureProcessor<AtomRmlFeatureProcessorInterface>();

            if (fp)
            {
                ImGui::Text("Child Pass Info:");
                fp->GetChildPasses([](AtomRmlChildPass* child)
                {
